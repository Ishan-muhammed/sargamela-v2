...
/**
 * Calculate points breakdown for a participant using Payload Local API
 * Returns total points and counts for each position
 */
export async function calculateParticipantPointsBreakdown(
  payload: Payload,
  ...
  try {
    ...
    console.error('Error calculating participant points breakdown:', error)
    return {
      total: 0,
      first: 0,
      second: 0,
